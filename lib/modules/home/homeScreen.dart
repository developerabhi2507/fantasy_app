...
class _HomeScreenState extends State<HomeScreen>
  ...
  Future<Null> getTeamData(bool pool) async {
    ...
    await authController.matchesRecord();

    // var responseData = await authController.postScheduleList();
  ...
  Widget listItems() {
    return Column(
      children: List.generate(
        authController.matchRecord!.data.fixtureMatch.length,
        (index) => MatchesList(
            matchId:
                authController.matchRecord!.data.fixtureMatch[index].matchId,
            titel: authController.matchRecord!.data.fixtureMatch[index].title,
            country1Name:
                authController.matchRecord!.data.fixtureMatch[index].teamName1,
            country2Name:
                authController.matchRecord!.data.fixtureMatch[index].teamName2,
            country1Flag:
                authController.matchRecord!.data.fixtureMatch[index].teamImage1,
            country2Flag:
                authController.matchRecord!.data.fixtureMatch[index].teamImage2,
            price: "₹2 Lakhs",
            time: authController.matchRecord!.data.fixtureMatch[index].time
                .toString()),
      ),
      // children: <Widget>[
...
class _MatchesListState extends State<MatchesList> {
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      onTap: () async {
        await authController.contestsList('7');
        Navigator.push(
          context,
          MaterialPageRoute(
